...
int main(int argc, char *argv[])
{
    ...
    printf("     -------  -------  --------  -------  -------  \n");
    for (int i=0; i<266; i++) {
        Vec ki = { arr[i*5+0], arr[i*5+1] };        // known input
        Vec ao = { ki[0], ki[1] };                  // actual output
        Flt ret = vecNormalize(ao);                 // normalize known input
        Flt tmp = ret - arr[i*5+2];                 // normalized vs act len
        tmp = fabs(tmp);
